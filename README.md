...
•Inherent Robustness: In contrast, DenseNet-121 demonstrated superior stability. Its accuracy decreased only from 92% to 88% (a minor 4% drop), indicating that its dense connectivity pattern may provide greater resistance to perturbations introduced by FGSM noise.

•Effectiveness of Adversarial Training: The research successfully "hardened" the vulnerable ResNet-50 model. By implementing adversarial training, the model's performance under attack was restored from 70% to 85%, representing a 15% increase in reliability.
